...
import { getItemById, getItems } from "../../../Functions";
...
export default function GalleryImages({ handleExpand }) {
  ...
  //     const response = await getItemById("art", i + 1)
  //     if (response) {
  ...
  //   }
  //   setImages(imagesArr)
  //   const response = await getItems("art")
  ...
  function handleClick() {
    ...
    for (let i = 0; i < allImages.length; i++) {
      const titleArr = allImages[i].title
        .split(" ")
        .map((el) => onlyAlph(toLower(el)));
      const artistArr = allImages[i].artist
        .split(" ")
        .map((el) => onlyAlph(toLower(el)));
      const mediumArr = allImages[i].medium
        .split(" ")
        .map((el) => onlyAlph(toLower(el)));
      if (
        titleArr
        ...
        titleIndexes = searchTerm
          ...
          .filter((el2) => el2 >= 0);
        titleResults = newImages[i].title
          .split(" ")
      ...
      if (
        ...
        matchImage = newImages[i];
        artistIndexes = searchTerm
          .map((el) => artistArr.indexOf(el))
          .filter((el2) => el2 >= 0);
        artistResults = newImages[i].artist
          .split(" ")
          .filter((el, ind) => artistIndexes.includes(ind));
        let newArtist = highlightMatch(
          newImages[i].artist,
          artistIndexes,
          artistResults
        );
      ...
      if (
        ...
        matchImage = newImages[i];
        mediumIndexes = searchTerm
          .map((el) => mediumArr.indexOf(el))
          .filter((el2) => el2 >= 0);
        mediumResults = newImages[i].medium
          .split(" ")
          .filter((el, ind) => mediumIndexes.includes(ind));
        let newMedium = highlightMatch(
          newImages[i].medium,
          mediumIndexes,
          mediumResults
        );
  ...
  return (
    <div className="galleryPage-results">
